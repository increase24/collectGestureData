...
struct detRect
{
    ...
    cv::Point2f center;
    detRect(cv::Mat &src, cv::Mat_<float> &mat, cv::Point2f &point) : img(src), affineMat(mat), center(point)
    {
    }
};
...
struct detMeta
{
    ...
    detMeta(int x_min, int y_min, int x_max, int y_max,
            cv::Point2f &Up, cv::Point2f &Down, int type = 0, int id = 0) : fid(id), xmin(x_min), ymin(y_min), xmax(x_max), ymax(y_max), detType(type), handUp(Up), handDown(Down)
    {
        if (type == 0)
        {
...
int main()
{
    ...
    string testImg = rootDir + "call.jpg";
    ...
    Ort::Session sessionPalm(env, palmModelw.c_str(), session_options);
    Ort::Session sessionHand(env, handModelw.c_str(), session_options);
